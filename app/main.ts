import "./styles.css";
import { inferCurrency, inferDate, inferMerchant, inventoryFromBackup, inventoryToCsv, isInventoryItem, parseReceiptText, redactPayment, type DraftLine, type InventoryItem } from "./lib";

type View = "intake" | "inventory" | "license";
interface Draft { receiptId: string; receiptName: string; merchant: string; purchaseDate: string; currency: string; lines: DraftLine[]; }
...
const RECEIPT_USAGE_KEY = demoMode ? DEMO_RECEIPT_USAGE_KEY : REAL_RECEIPT_USAGE_KEY;
const demoItems: InventoryItem[] = [
  { id: "demo-kettle", receiptId: "demo-home-shop", receiptName: "home-shop.jpg", name: "Cedar kettle", quantity: 1, price: 42, currency: "USD", confidence: 96, confidenceLabel: "good", included: true, merchant: "Home Store", room: "Kitchen", category: "Appliance", purchaseDate: "2026-08-21", warrantyDate: "2028-08-21", createdAt: "2026-08-21T10:00:00.000Z" },
  { id: "demo-lamp", receiptId: "demo-home-shop", receiptName: "home-shop.jpg", name: "Reading lamp", quantity: 1, price: 39, currency: "USD", confidence: 91, confidenceLabel: "good", included: true, merchant: "Home Store", room: "Office", category: "Decor", purchaseDate: "2026-08-21", warrantyDate: "", createdAt: "2026-08-21T10:00:00.000Z" },
  { id: "demo-box", receiptId: "demo-linen-shop", receiptName: "linen-shop.jpg", name: "Linen storage box", quantity: 1, price: 12.5, currency: "USD", confidence: 88, confidenceLabel: "good", included: true, merchant: "Linen Shop", room: "Bedroom", category: "Home supply", purchaseDate: "2026-08-24", warrantyDate: "", createdAt: "2026-08-24T11:00:00.000Z" }
];
...
let licenseValid = !demoMode && readLicenseCache()?.valid === true && Boolean(localStorage.getItem(LICENSE_KEY));
let fileQueue: File[] = [];
...
function loadItems(): InventoryItem[] {
  try {
    ...
    if (!Array.isArray(parsed) || !parsed.every(isInventoryItem)) throw new Error("Invalid stored inventory");
    return parsed;
  } catch {
...
function freeLimitReached(): boolean { return !demoMode && !licenseValid && receiptCount() >= FREE_RECEIPTS; }
...
function render(focusSelector?: string): void {
  document.title = view === "inventory" ? "Inventory — Receipt to Room" : view === "license" ? "License — Receipt to Room" : "Add receipt — Receipt to Room";
  app.innerHTML = `
...
function bindIntake(): void {
  ...
  document.querySelector("#parse-manual")?.addEventListener("click", () => {
    ...
    if (freeLimitReached()) { showLimit(); return; }
    manualEntryOpen = false; manualEntryInvalid = false; createDraft("Typed receipt", text, 100); render("#review-title");
  });
  ...
  document.querySelector("#add-line")?.addEventListener("click", () => {
    syncDraftFromForm(); draft?.lines.push({ id: crypto.randomUUID(), name: "", quantity: 1, price: 0, confidence: 100, confidenceLabel: "good", included: true, room: "Kitchen", category: "Home supply", warrantyDate: "" }); render();
...
async function processFiles(files: File[]): Promise<void> {
  ...
  const allowance = demoMode || licenseValid ? valid.length : Math.max(0, FREE_RECEIPTS - receiptCount());
...
function createDraft(name: string, text: string, confidence: number, ocrLines?: Array<{ text: string; confidence: number }>): void {
  if (freeLimitReached()) { showLimit(); return; }
  const parsed = ocrLines?.length ? ocrLines.flatMap((line) => parseReceiptText(line.text, line.confidence)) : parseReceiptText(text, confidence);
  const lines = parsed.map((line) => ({ ...line, room: "Kitchen", category: "Home supply", warrantyDate: "" }));
  draft = { receiptId: crypto.randomUUID(), receiptName: name, merchant: inferMerchant(text), purchaseDate: inferDate(text), currency: inferCurrency(text), lines };
}
...
function syncDraftFromForm(): void {
  ...
  draft.merchant = String(data.get("merchant") ?? ""); draft.purchaseDate = String(data.get("purchaseDate") ?? ""); draft.currency = String(data.get("currency") ?? "USD");
  draft.lines.forEach((line, i) => {
    line.included = data.has(`included-${i}`); line.name = String(data.get(`name-${i}`) ?? "").trim(); line.quantity = Number(data.get(`quantity-${i}`) ?? 1); line.price = Number(data.get(`price-${i}`) ?? 0);
    line.room = String(data.get(`room-${i}`) ?? "Other"); line.category = String(data.get(`category-${i}`) ?? "Other"); line.warrantyDate = String(data.get(`warrantyDate-${i}`) ?? "");
  });
}
...
function acceptDraft(event: SubmitEvent): void {
  ...
  items.push(...chosen.map((line) => ({ ...line, receiptId: draft!.receiptId, receiptName: draft!.receiptName, merchant: draft!.merchant, currency: draft!.currency, purchaseDate: draft!.purchaseDate, createdAt: now })));
  receiptUsage += 1; saveReceiptUsage(); saveItems(); status = `${chosen.length} item${chosen.length === 1 ? "" : "s"} added to the room inventory.`; draft = null; error = "";
...
function bindInventory(): void {
  ...
  document.querySelector("#cancel-edit")?.addEventListener("click", () => { editingItemId = null; render("main h1"); });
  document.querySelector<HTMLFormElement>("#edit-item-form")?.addEventListener("submit", saveEditedItem);
}
...
function saveEditedItem(event: SubmitEvent): void {
  ...
  if (!name || !Number.isInteger(quantity) || quantity < 1 || quantity > 999 || !Number.isFinite(price) || price < 0) {
    status = "Enter a name, a quantity from 1 to 999, and a non-negative price.";
    return;
  }
  items[index] = { ...items[index], name, quantity, price, currency: String(data.get("currency")), room: String(data.get("room")), category: String(data.get("category")), purchaseDate: String(data.get("purchaseDate")), warrantyDate: String(data.get("warrantyDate")), merchant: String(data.get("merchant")).trim() };
  saveItems();
  editingItemId = null;
  status = `${name} updated.`;
  render("main h1");
...
function bindLicense(): void {
  ...
  document.querySelector<HTMLInputElement>("#restore-json")?.addEventListener("change", async (e) => {
    const file = (e.target as HTMLInputElement).files?.[0]; if (!file) return;
    try {
      const restored = inventoryFromBackup(JSON.parse(await file.text()) as unknown);
      if (!restored) throw new Error("Invalid backup schema");
      items = restored;
      receiptUsage = Math.max(receiptUsage, new Set(items.map((item) => item.receiptId)).size);
      saveReceiptUsage(); saveItems(); status = `Restored ${items.length} items.`;
      navigateTo("inventory");
    } catch { status = "That file is not a valid Receipt to Room backup. Your current records were kept."; render(); }
  });
}
...
window.addEventListener("hashchange", () => { view = viewFromLocation(); editingItemId = null; render("main h1"); });
